...
splits = 5
angleClasses = 5
features = numpy.zeros([len(images), splits * splits * angleClasses])
for i in range(amount):
    print(i, "/", amount)
    features[i] = extractor.splitAngleFeatures(thumbs[i],splits,angleClasses)
    #features[i] = extractor.quadrantAngleFeatures(thumbs[i], angleClasses)
